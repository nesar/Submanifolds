...
file_eval = dLoad+'Evals3_050_'+str(sig)+'.npy'
l = np.load(file_eval)
...
alpha = 0.8
plt.sca(ax[0])
plt.imshow(nstream[sliceNo,:,:], norm=LogNorm(vmin, vmax) , cmap = cmap, alpha = alpha)
...
fl = plt.imshow(nstream[sliceNo,:,:], norm=LogNorm(vmin, vmax) , cmap = cmap, alpha = 0.3)
cbar = plt.colorbar(fl, orientation='horizontal',  pad = 0.1, shrink = 0.8, ticks= np.logspace(0, vmax,3) )

plt.savefig('plots/nstrHalo100_slice'+str(sliceNo)+'_sig'+str(sig)+'.png')
...
np.save('npy/100Mpc128/particlesL3HaloQ', np.vstack((q0_labeled, q1_labeled, q2_labeled, haloID)))
...
toPara.Points1d(q0_labeled*size_fact/L , q1_labeled*size_fact/L , q2_labeled*size_fact/L , haloID, 'vti/LabeledL3_051Q')
