...
sigList = [0.0, 1.0, 2.0, 5.0, 10.0, 20.0]
sig = sigList[2]
...
fileIn = 'npy/'+dir1+'flip_snap_050.npy'

flip = np.load(fileIn).astype(np.float64)

#flip = flip[0:10,0:10,0:10]

flip = Box(flip,2).extendBox()

from scipy import ndimage
flip = ndimage.gaussian_filter(flip, sigma= sig)

dx = dy = dz = L/size_fact

#dx = dy = dz = 1

Nx, Ny, Nz = np.gradient(-flip, dx,dy,dz)
...
np.save("npy/"+dir1+"ffEvals3_050_"+str(sig), l_new)
#np.save("npy/ffEvals3_051", l_new)
#np.save("npy/ffEvec1_051", v1_new)
#np.save("npy/ffEvec2_051", v2_new)
#np.save("npy/ffEvec3_051", v3_new)
