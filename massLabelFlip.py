...
x0_1d = np.ravel(q0_3d)
x1_1d = np.ravel(q1_3d)
x2_1d = np.ravel(q2_3d)
...
file_eval = 'npy/ffEvals3_051_'+str(sig)+'.npy'   # include softening
...
#labels1a = labels1
#labels1a = VoidParticleUnbind(labels1, x0_3d, x1_3d, x2_3d)   # Unbind particles in ff = 1 
...
plt.title(r'x = %4.1f kpc'%(1000.*sliceNo/128))
...
plt.title(r'$\sigma$ = %4.1f kpc'%(1000*sig/128))
...
#plt.savefig('plots/ffHalo_slice'+str(sliceNo)+'_sig'+str(sig)+'.png')
...
np.save('npy/ffparticlesL3Halo', np.vstack((x0_labeled, x1_labeled, x2_labeled, haloID)))
np.save('npy/fflabels0', labels0)
