...
λth = 0.0
Can find λth by using void fraction from flip = 1 regions. 
...
#------------------------------------------------------------------------------ 
def LabelCutStr(labels3d, ffCutMax, ffCutMin):
    
    labels1d = np.unique(labels3d)[1:]
    
    print '---------------------------------' 
    print 'Max flip threshold', ffCutMax
    print 'Min flip threshold', ffCutMin
    print '#Haloes before flips-cut:', labels1d.shape

    
    maxffEachBlob = np.array(ndi.maximum(flip, labels=labels3d, index= labels1d))
    c1 = (maxffEachBlob < ffCutMax)   #max(flip) = 1  ( entire region in void)

    minffEachBlob = np.array(ndi.minimum(flip, labels=labels3d, index= labels1d))
    c2 = (minffEachBlob < ffCutMin)   #max(flip) = 1  ( entire region in void)

    MaskOutCondition0 = np.where( c1 | c2 )
    maskingValues = labels1d[MaskOutCondition0]
    labels3d_out = MaskedRemove(labels3d, maskingValues)
    
    print '#Haloes after flips-cut flips:', (np.unique(labels3d_out)[1:]).shape
    


#    maxffEachBlob = np.array(ndi.maximum(flip, labels=labels3d_out, index= (np.unique(labels3d_out)[1:])))
#    minffEachBlob = np.array(ndi.minimum(flip, labels=labels3d_out, index= (np.unique(labels3d_out)[1:])))
#
#    print 'Streams min(min) ', minffEachBlob.min()
#    print 'Streams min(max) ', maxffEachBlob.min()
...
def LabelCut(labels3d, ffCut, massCut, volCut, densCut):
    ...
    maxffEachBlob = np.array(ndi.maximum(flip, labels=labels3d, index= labels1d))
    c1 = (maxffEachBlob < ffCut)   #max(flip) = 1  ( entire region in void)
...
def massParticleCut(labels3d, massCut):
    particleLabel1d = ParticleLabel(labels3d, q0_1d, q1_1d, q2_1d)
...
### #remove particles if they belong to flip = 1 
    ...
    regionLabels[flip[x0, x1, x2] == 1] = 0
...
q0_1d = np.ravel(q0_3d)
q1_1d = np.ravel(q1_3d)
q2_1d = np.ravel(q2_3d)
...
#flip = np.load(file_ff).astype(np.float64)

file_eval = 'npy/'+dir1+'ffEvals3_050_'+str(sig)+'.npy'   # include softening
#np.save("npy/ffEvals3_051_"+str(sig), l_new)
...
labels1 = LabelCutStr(labels, ffCutMax, ffCutMin)      #flip cut 

#labels1a = labels1
#labels1a = VoidParticleUnbind(labels1, q0_3d, q1_3d, q2_3d)   # Unbind particles in ff = 1 
...
#labels0 = LabelCut(labels1, ffCutMax, massCut, volCut, densCut)  #ff, density, volume cut
...
particleLabel1d = ParticleLabel(labels0, q0_1d, q1_1d, q2_1d)
labelidx = np.where(np.in1d( particleLabel1d , np.unique(labels0)[1:] ))[0]
q0_labeled = q0_1d[labelidx]
q1_labeled = q1_1d[labelidx]
q2_labeled = q2_1d[labelidx]
...
plt.clf()
f, ax = plt.subplots(1,3, figsize = (13.5,6), sharey = True)
...
bw = colors.ListedColormap(['white', 'gray'])
sliceNo = 70
vmin = 1
vmax = 1000
alpha = 0.8
plt.sca(ax[0])
plt.imshow(flip[sliceNo,:,:], norm=LogNorm(vmin, vmax) , cmap = cmap, alpha = alpha)
plt.title(r'x = %4.1f Mpc'%(100.*sliceNo/128))
cbar = plt.colorbar(orientation='horizontal',  pad = 0.1, shrink = 0.8, ticks= np.logspace(0, vmax,3) )
#cbar.set_ticklabels([r'$-$', r'$0$', r'$+$'])

plt.sca(ax[1])
plt.imshow(labels0[sliceNo,:,:]>0, cmap = bw)
plt.title(r'$\sigma$ = %4.1f Mpc'%(100*sig/128))
cbar = plt.colorbar(orientation='horizontal',  pad = 0.1, shrink = 0.8, ticks=[ 0, 1])


plt.sca(ax[2])
plt.imshow(labels0[sliceNo,:,:]>0, alpha = 1.0, cmap = bw)
fl = plt.imshow(flip[sliceNo,:,:], norm=LogNorm(vmin, vmax) , cmap = cmap, alpha = 0.3)
cbar = plt.colorbar(fl, orientation='horizontal',  pad = 0.1, shrink = 0.8, ticks= np.logspace(0, vmax,3) )

plt.savefig('plots/ffHalo100_slice'+str(sliceNo)+'_sig'+str(sig)+'.png')
...
np.save('npy/100Mpc128/ffparticlesL3Halo', np.vstack((q0_labeled, q1_labeled, q2_labeled, haloID)))
np.save('npy/100Mpc128/ffparticlesL3HaloX', np.vstack((x0_labeled, x1_labeled, x2_labeled, haloID)))

np.save('npy/100Mpc128/fflabels0', labels0)
    
toPara.Points1d(q0_labeled*size_fact/L , q1_labeled*size_fact/L , q2_labeled*size_fact/L , haloID, 'vti/ffLabeledL3_051')
toPara.Points1d(x0_labeled*size_fact/L , x1_labeled*size_fact/L , x2_labeled*size_fact/L , haloID, 'vti/ffLabeledL3_051X')

toPara.StructuredScalar(labels0, 'vti/ffl3_051_'+str(refFactor), 0, size_fact)
toPara.StructuredScalar(flip, 'vti/ff_051_'+str(refFactor), 0, size_fact)
...
def StatsLabels(labels3d): 
    ...
    #strEachBlob = ndi.measurements.sum(flip, labels= labels3d, index= label1d )
    
    maxffEachBlob = np.array(ndi.maximum(flip, labels=labels3d, index= label1d))
    minffEachBlob = np.array(ndi.minimum(flip, labels=labels3d, index= label1d))
    
    PDFplot(maxffEachBlob,  r'max($n_{str}$) in Halo')
    PDFplot(minffEachBlob,  r'min($n_{str}$) in Halo')
    
    gridOnes = np.ones_like(flip)       #  RefinedGrid
...
def PDFplot(array1d, strLabel):
    ...
    plt.savefig('plots/ff'+strLabel+'.pdf', bbox_inches='tight')
...
flip_2d = flip[slice_noX,ybeg:yend,zbeg:zend]

flip_2d = np.where(flip[slice_noX,ybeg:yend,zbeg:zend] > 1,  1 , 0)

#flip_2d = np.where(flip[slice_noX,ybeg:yend,zbeg:zend] > 1, 1, 0)
#cmap = colors.ListedColormap(['white', 'grey', 'red'])
#bounds= np.array([0, ffCutMin, ffCutMax]) + 0.5
#norm = colors.BoundaryNorm(bounds, cmap.N)
#im2 = plt.imshow(flip_2d,  alpha = 0.5, cmap = cmap , norm = norm)

im2 = ax[0, 1].imshow(flip_2d,  alpha = 0.3, cmap = 'cubehelix_r' )
...
plt.savefig('plots/fflabels123.pdf', bbox_inches='tight')
...
im2 = plt.imshow(flip_2d,  alpha = 0.3, cmap = 'cubehelix_r' )
...
plt.savefig('plots/fflabels.pdf', bbox_inches='tight')
...
plt.savefig('plots/ffEvals123.pdf', bbox_inches='tight')
...
plt.savefig('plots/ffminmaxStrInHalo.pdf', bbox_inches='tight')
plt.show()
